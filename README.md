...
* [`src/tgz_tools/`](src/tgz_tools/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/tgz_tools/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `tgz-tools` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/tgz_tools/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns tgz-tools.example
  (:require [tgz-tools.config :as config])
